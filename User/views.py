...
from .forms import UserForm, loginForm, ProfileForm, Check_code
import re
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            context = {'username': username, 'pwd': password, 'email': email, 'phone': phone}
            if password.isdigit():
            ...
            # 判断用户是否存在
            phone = Users.objects.filter(phone=phone)  # 判断此号码是否注册过
...
def Code_test(request):
    if request.method == 'POST':
        ...
        taken = data['taken']  # 用户登录后生成的taken信息
        code = data['code']  # 用户提交的代码
        time = data['time']  # 提交时间
        check = Check_code(username, taken, code, time)
        t = check.check_code()
        if t:
            arr.append(check)
        else:
